...
import { updateAdSet } from "@/lib/meta";
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const input: { name?: string; status?: "ACTIVE" | "PAUSED"; dailyBudget?: number } = {};
    if (body.name !== undefined) {
      if (typeof body.name !== "string" || !body.name.trim()) {
        return jsonError("Geçerli bir reklam seti adı gerekli", 400);
      }
    ...
    if (body.dailyBudget !== undefined) {
      ...
      input.dailyBudget = body.dailyBudget;
    }
    if (Object.keys(input).length === 0) {
      return jsonError("Güncellenecek alan belirtilmedi", 400);
    }

    const adset = await updateAdSet(id, input);
